<?php

return [
    "listeing_created" => "تمت إضافة القائمة بنجاح . . .",
    "listeing_updated" => "تم تحديث القائمة بنجاح . . .",
    "listeing_deleted" => "تم حذف القائمة بنجاح . . .",
    "image_uploaded" => "تم تحميل الصورة بنجاح . . .",
    "image_deleted" => "تم حذف الصورة بنجاح . . .",
    "marked_as_read" => "تم تحديد الاشعارات كمقروءة بنجاح . . .",
    "offer_created" => "تم انشاء العرض بنجاح . . .",
    "offer_accepted" => "تم قبول العرض بنجاح . . .",
    'listeing_restored' => 'تم استعادة القائمة بنجاح . . .',
    'account_created' => 'تم انشاء الحساب بنجاح . . .',
    'success' => 'تمت العملية بنجاح . . .',
    'account_inactive' => 'حسابك غير مفعل . . .',
    'otp_sent' => 'تم ارسال الكود بنجاح . . .',
    'invalid_otp' => 'كود غير صحيح . . .',
];
